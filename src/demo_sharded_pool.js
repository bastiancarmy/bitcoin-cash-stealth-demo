...
import { bytesToHex, hexToBytes, sha256, _hash160, ensureEvenYPriv } from '@bch/utils';
import {
  broadcastTx,
  getUtxos,
  getTxDetails,
...
import { RPA_MODE_STEALTH_P2PKH, deriveRpaLockIntent, deriveRpaOneTimePrivReceiver } from '@bch/rpa';
import { POOL_HASH_FOLD_VERSION, getPoolHashFoldBytecode, computePoolStateOut } from '@bch/pool-hash-fold';
import { initShards, depositToShard, importShardNotes, withdrawFromShard } from '@bch/pool-shards';
...
function recordFoundStealth(state, ownerTag, purpose, matches) {
  for (const m of matches) {
    upsertStealthUtxo(state, {
      owner: ownerTag,
      purpose,
      txid: m.txid,
      vout: m.vout,
      value: m.value,
      hash160Hex: m.hash160Hex,
      rpaContext: m.rpaContext,
...
async function ensurePoolState({ ownerWallet, ownerPaycodePub33, shardCount, poolVersion, fresh = false }) {
  ...
  const init = await initShards({ state, ownerWallet, ownerPaycodePub33, shardCount, poolVersion });
...
async function sweepDepositDebug({ depositOutpoint, receiverWallet }) {
  ...
  if (!ctx?.senderPub33Hex || !ctx?.prevoutHashHex) {
    throw new Error('depositOutpoint missing rpaContext');
  }
  ...
  if (bytesToHex(h160) !== bytesToHex(expectedH160)) {
    throw new Error(
      `sweep derivation mismatch. expected=${bytesToHex(expectedH160)} derived=${bytesToHex(h160)}`
    );
  }
  ...
  // sanity check: rawTx must literally contain the scriptSig bytes we think it does
...
async function ensureDeposit({ state, senderWallet, senderPaycodePub33, senderTag = 'alice', receiverPaycodePub33, amountSats, fresh = false }) {
  // reuse newest unimported deposit of this amount if it’s still unspent
  if (!fresh) {
...
async function ensureImport({ state, receiverWallet, shardIndexOpt = null, fresh = false }) {
  ...
  // If already imported (according to state), treat as done.
  ...
  // If not seen yet, wait a bit (mempool/indexing lag is common)
  if (!stillUnspent) {
  ...
  // If still not visible via scripthash, allow recent deposits to proceed (0-conf path)
  if (!stillUnspent) {
    const ageMs =
      dep.createdAt ? Date.now() - Date.parse(dep.createdAt) : Number.POSITIVE_INFINITY;
  ...
  // derive shard index unless explicitly provided
  ...
  // Update deposit record in-place (avoid duplicates)
  upsertDeposit(state, {
    ...dep,
...
async function ensureWithdraw({
  ...
  // If state already reflects a withdrawal result for this shard+amount+receiver and the shard is already at shardAfter, skip.
  ...
  // Enrich last withdrawal entry with before/after + receiver paycode (for idempotence checks)
...
async function tryRepairShardsFromWallet({ state, ownerWallet }) {
  ...
  // Normalize category coming from server (can be string OR Uint8Array depending on server/parser)
  ...
  for (const u of tokenUtxos) {
    try {
      ...
      const commitmentHex =
        commitment instanceof Uint8Array ? bytesToHex(commitment) : 'UNKNOWN';
  ...
  // NOTE: We cannot recover original shard indices once commitments have been mutated.
  // We'll assign indices in the order found. (Good enough for demo idempotence.)
  const repairedShards = matches.map((m, i) => ({ index: i, ...m }));

  const unknown = repairedShards.filter((s) => s.commitmentHex === 'UNKNOWN').length;
  if (unknown) {
    console.warn(`repair: ${unknown} shard commitments missing; outpoints recovered only.`);
  }
...
async function pickFeeRateOrFallback() {
  try {
    const fr = await getFeeRate(); // your electrum.js returns sat/byte
    if (typeof fr === 'number' && Number.isFinite(fr) && fr >= 1) return Math.ceil(fr);
  } catch {}
  // chipnet relay floor in your electrum.js is 2, but keep demo safe:
  return 2;
}

function feeForSizeOrDefault(sizeBytes) {
  // Keep it simple and safe in a demo.
  return BigInt(Math.max(1_000, Math.ceil(sizeBytes)));
}

async function consolidateWallet(wallet, { optional = false } = {}) {
  if (!wallet?.address || !wallet?.privBytes) {
    throw new Error('Wallet must include address and privBytes for UTXO consolidation.');
  }
  return consolidateUtxos(wallet.address, wallet.privBytes, NETWORK, optional);
}
...
function upsertStealthUtxo(state, rec) {
  ...
  if (idx >= 0) {
    state.stealthUtxos[idx] = { ...state.stealthUtxos[idx], ...rec };
  } else {
    state.stealthUtxos.push(rec);
  }
}
...
async function finalizeAndSignInitTx({ tx, inputPrivBytes, prevOut }) {
  // signing affects size, so sign first
  ...
  // electrum broadcast expects hex
...
async function initShards({ state = null, ownerWallet, ownerPaycodePub33 = null, shardCount, poolVersion }) {
  const redeemScript = await getPoolHashFoldBytecode(poolVersion);

  const shardsTotal = SHARD_VALUE * BigInt(shardCount);

  // Prefer any previously-created stealth UTXOs for Bob (so Bob's base address stays quiet).
  // If state is null (e.g. first ever init), this naturally falls back to base P2PKH UTXOs.
  const funding = await selectFundingUtxo({
    state,
    wallet: ownerWallet,
    ownerTag: ACTOR_B.id,
    // shardsTotal + dust + a little headroom for fees
    minSats: shardsTotal + BigInt(DUST) + 20_000n,
  });

  const prev = funding.prevOut;

  // Token category convention used elsewhere in this repo: reverse(prevout.txid).
  const category32 = reverseBytes(hexToBytes(funding.txid));

  const inputValue = toBigIntSats(prev.value);

  // Build N shard outputs.
  ...
  // Change output: prefer stealth P2PKH (RPA) back to the owner paycode if available.
  ...
  // feeRate is sat/byte (your getFeeRate already enforces chipnet >= 2)
  const feeRate = await pickFeeRateOrFallback();

  // Start with a conservative guess so the tx is well-formed for first signing/size measurement.
  let feeGuess = 8_000n;
  ...
  // Assemble tx with guessed change
  const tx = {
    version: 2,
    locktime: 0,
    inputs: [
  ...
  // Compute required fee from signed size + feeRate (+ safety)
  const feeNeeded = feeFromSize(sizeBytes, feeRate, { safety: 200n });

  // If our guess underpaid, update change and re-sign once.
...
` +
  ...
  // ---- Logging: where init change went ----
  if (changeStealthTemplate) {
    ...
    console.log(
      `[init] change (base P2PKH fallback) -> outpoint ${txid}:${shardCount} value=${changeValue.toString()} sats`
    );
  }
...
async function createDeposit({
  ...
  // Pick a single funding UTXO (prefer recorded stealth UTXOs).
  ...
  // Derive receiver stealth output (index 0) from the actual spent prevout.
  ...
  // Derive stealth change back to sender paycode (index 1).
  ...
  if (changeValue >= BigInt(DUST)) {
    ...
    // No change output; the remainder becomes fee. This avoids creating a dust output.
    changeValue = 0n;
  }
  ...
  // Sign with either the base privkey or the one-time stealth privkey.
  signInput(tx, 0, senderUtxo.signPrivBytes, prev.scriptPubKey, BigInt(prev.value));

  const rawTx = buildRawTx(tx);
  const txid = await broadcastTx(rawTx);

  // ---- Logging: deposit outputs ----
  console.log(
  ...
  if (changeRec && changeValue > 0n) {
    ...
    console.log(
      `[deposit] change: none (remainder absorbed into fee to avoid dust)`
    );
  }

  // If we spent a recorded stealth UTXO, mark it spent for idempotence.
  if (senderUtxo.source === 'stealth') {
    markStealthSpent(state, senderUtxo.txid, senderUtxo.vout, txid);
  }

  // Finalize change record now that txid is known.
  ...
  return {
    deposit: {
      ...
      createdAt: new Date().toISOString(),
      // needed for receiver to derive one-time priv
...
async function importDepositToShard({ poolState, shardIndex, depositOutpoint, receiverWallet }) {
  ...
  const depositPrev = await getPrevOutput(depositOutpoint.txid, depositOutpoint.vout).catch(async (e) => {
    // Electrum indexing can lag. If deposit is recent, fall back to stored metadata.
  ...
  // ---- Deterministic one-time key derivation (LOCKED-IN) ----
  const expectedH160 = parseP2pkhHash160(depositPrev.scriptPubKey);
  if (!expectedH160) throw new Error('deposit prevout is not P2PKH (unexpected for demo)');

  const ctx = depositOutpoint.rpaContext;
  if (!ctx?.senderPub33Hex || !ctx?.prevoutHashHex) {
    throw new Error('depositOutpoint missing rpaContext (cannot spend stealth P2PKH)');
  }

  const senderPub33 = hexToBytes(ctx.senderPub33Hex);

  // Helpful preflight logs (do NOT branch on these).
  try {
  ...
  // proof blob content not used by covenant; only length-checked+dropped
  ...
  const tokenOut = {
    category: category32, // IMPORTANT: do NOT vary token category on-chain
  ...
  // sign deposit input (index 1)
  signInput(tx, 1, oneTimePriv, depositPrev.scriptPubKey, BigInt(depositPrev.value));

  const rawTx = buildRawTx(tx);

  // sanity-check: output token commitment matches computed commitment
...
async function withdrawFromShard({
  ...
  // Fee input (prefer stealth funding to keep base address quiet)
  ...
  // Receiver payment output (stealth P2PKH), derived from the actual spent fee prevout.
  const { intent: payIntent, rpaContext: payContext } = deriveStealthP2pkhLock({
    senderWallet,
    receiverPaycodePub33,
  ...
  // Stealth change back to sender paycode.
  const { intent: changeIntent, rpaContext: changeContext } = deriveStealthP2pkhLock({
    senderWallet,
    receiverPaycodePub33: senderPaycodePub33,
    prevoutTxidHex: feeUtxo.txid,
  ...
  // Fold a "nullifier-ish" hash into state. (Future: ZK spend reveals a nullifier.)
  ...
  const limbs = []; // placeholder for batch-spend / script fingerprints / etc.
  ...
  if (changeValue >= BigInt(DUST)) {
    ...
    // Avoid a dust output; remainder becomes additional fee.
    changeValue = 0n;
  }
  ...
  if (changeRec && changeValue > 0n) {
    ...
    console.log(
      `[withdraw] change: none (remainder absorbed into fee to avoid dust)`
    );
  }

  // Mark spent stealth input (if applicable)
  if (feeUtxo.source === 'stealth') {
    markStealthSpent(poolState, feeUtxo.txid, feeUtxo.vout, txid);
  }

  // Record stealth change output (if any)
  if (changeRec) {
    changeRec.txid = txid;
  ...
  poolState.withdrawals = poolState.withdrawals ?? [];
...
async function loadDemoActors() {
  const ACTOR_A = { id: 'actor_a', label: 'Alice' };
  const ACTOR_B = { id: 'actor_b', label: 'Bob' };
  const wallets = await getWallets();
  const aliceBaseWallet = wallets.alice;
  const bobBaseWallet = wallets.bob;

  if (!aliceBaseWallet?.pubBytes || !bobBaseWallet?.pubBytes) {
    throw new Error(
      `getWallets() returned unexpected shape. Keys: ${Object.keys(wallets ?? {}).join(', ')}`
    );
  }

  // ✅ paycodes.js expects (alice, bob), not a single object
  const { alicePaycode, bobPaycode } = setupPaycodesAndDerivation(aliceBaseWallet, bobBaseWallet);

  const alicePaycodePub33 = extractPubKeyFromPaycode(alicePaycode);
  const bobPaycodePub33 = extractPubKeyFromPaycode(bobPaycode);
...
program
  ...
  .action(async (opts) => {
    ...
    const { bobBaseWallet, bobPaycodePub33 } = await loadDemoActors();

    const poolVersion =
      program.opts().poolVersion === 'v1' ? POOL_HASH_FOLD_VERSION.V1 : POOL_HASH_FOLD_VERSION.V1_1;

    const init = await initShards({ state: null, ownerWallet: bobBaseWallet, ownerPaycodePub33: bobPaycodePub33, shardCount, poolVersion });
...
program
  ...
  .action(async (opts) => {
    ...
    const { aliceBaseWallet, alicePaycodePub33, bobPaycodePub33 } = await loadDemoActors();

    await ensureDeposit({
      state,
      senderWallet: aliceBaseWallet,
      senderPaycodePub33: alicePaycodePub33,
      senderTag: ACTOR_A.id,
      receiverPaycodePub33: bobPaycodePub33,
      amountSats,
      fresh: !!opts.fresh,
    });
...
program
  ...
  .action(async (opts) => {
    ...
    if (opts.sweep) {
      ...
      const res = await sweepDepositDebug({ depositOutpoint: dep, receiverWallet: bobBaseWallet });

      // Optional but recommended: mark deposit as spent in state so it won't be reused
      upsertDeposit(state, { ...dep, spentTxid: res?.txid ?? 'unknown', spentAt: new Date().toISOString() });
      await writeState(state);
...
program
  ...
  .action(async (opts) => {
    ...
    const { bobBaseWallet, bobPaycodePub33, alicePaycodePub33 } = await loadDemoActors();

    await ensureWithdraw({
      state,
      shardIndex,
      amountSats,
      senderWallet: bobBaseWallet,
      senderPaycodePub33: bobPaycodePub33,
      senderTag: 'bob',
      receiverPaycodePub33: alicePaycodePub33,
      fresh: !!opts.fresh,
    });
...
program
  ...
  .action(async (opts) => {
    ...
    if (!Number.isFinite(shardCount) || shardCount < 2) {
      throw new Error('shards must be >= 2');
    }
    ...
    const {
      ...
      alicePaycodePub33,
      bobPaycodePub33,
    } = await loadDemoActors();

    const poolVersion =
      program.opts().poolVersion === 'v1'
        ? POOL_HASH_FOLD_VERSION.V1
        : POOL_HASH_FOLD_VERSION.V1_1;

    // [0/4] init/reuse (ensurePoolState already handles: reuse, validate, repair, or init)
    let state = await ensurePoolState({
      ownerWallet: bobBaseWallet,
      ownerPaycodePub33: bobPaycodePub33,
      shardCount,
      poolVersion,
      fresh: !!opts.fresh,
    });

    // [2/4] deposit (idempotent)
    console.log(`\n[2/4] deposit ${depositSats} sats (Alice -> Bob stealth P2PKH)...`);
    const dep = await ensureDeposit({
      state,
      senderWallet: aliceBaseWallet,
      senderPaycodePub33: alicePaycodePub33,
      senderTag: ACTOR_A.id,
      ...
      // run uses fresh only for init; deposit stays idempotent unless you add a separate flag
      fresh: false,
    });

    // [3/4] import (idempotent)
    console.log(`\n[3/4] import deposit into shard (derived selection)...`);
    const imp = await ensureImport({
      state,
      receiverWallet: bobBaseWallet,
      shardIndexOpt: null, // derived inside ensureImport()
      fresh: false,
    });

    const shardIndex = imp?.shardIndex ?? 0;

    // [4/4] withdraw (idempotent)
